...
- 服用趋势模型：至少 7 个有提醒日期后，按近 7 天完成率、前一周期变化、线性斜率、忽略/稍后比例和稳定度生成客观趋势；数据不足时不生成趋势判断。
- 剂量变化记录模型：记录旧剂量、新剂量、生效日期和用户确认备注；趋势模型会把剂量变化作为解释上下文，不把它解读为诊断、处方或疗效判断。
...
- 依从性按日统计、连续记录天数和补记修正。
- 服用趋势不足一周、改善、下降和平稳判断。
- 剂量变化记录作为服用趋势解释上下文，数据不足时仍保留剂量变化摘要。
...
当前新 Mac 测试结果：60 个 Swift Testing 测试全部通过。
