...
codex plugin add build-ios-apps@openai-curated || true
codex plugin add build-ios-apps@personal || true
...
6. XcodeBuildMCP 是否可启动并返回 `tools/list`。如果报 `env: node: No such file or directory`，给 Build iOS Apps 的 `.mcp.json` 中 `xcodebuildmcp.env` 补入：
   `PATH=/Applications/Codex.app/Contents/Resources/cua_node/bin:/usr/bin:/bin:/usr/sbin:/sbin`
...
Build iOS Apps / XcodeBuildMCP 已确认：

- MCP server 名称：`xcodebuildmcp`
- MCP transport：`stdio`
- MCP command：`/Applications/Codex.app/Contents/Resources/cua_node/bin/npx`
- MCP args：`-y xcodebuildmcp@latest mcp`
- 必须保留的 MCP env：
  - `PATH=/Applications/Codex.app/Contents/Resources/cua_node/bin:/usr/bin:/bin:/usr/sbin:/sbin`
  - `XCODEBUILDMCP_ENABLED_WORKFLOWS=simulator,ui-automation,debugging`
- 原因：`npx` 使用绝对路径仍不够，`xcodebuildmcp@latest` 启动后的包脚本会通过 `/usr/bin/env node` 查找 `node`。如果 MCP env 没有上面的 `PATH`，会失败为 `env: node: No such file or directory`。
- 2026-06-10 已验证 `xcodebuildmcp` version `2.6.2` 可通过 MCP stdio 初始化并注册 44 个 tools，关键工具包括 `session_show_defaults`、`list_sims`、`build_run_sim`、`build_sim`、`test_sim`、`snapshot_ui`、`screenshot`、`tap`、`type_text`、`wait_for_ui`。
- 如果已打开的旧线程仍看不到 `mcp__xcodebuildmcp` 工具，这是旧 turn 或旧线程工具表没有热刷新；重启 Codex、开新一轮、或新建/分叉线程后再试。
