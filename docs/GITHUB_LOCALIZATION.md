...
远端标签的重命名需要先更新仓库中的 Issue Form、文档和查询引用，再逐项更新 GitHub，并在每一步读回。下表是目标名称；迁移准备阶段，仓库模板同时列出当前英文名和目标中文名：目标中文标签尚不存在时，GitHub 只应用已存在的英文标签；重命名后只应用已存在的中文标签。这样可以避免迁移窗口内丢失模板默认标签。
...
3. 合并后先逐个更新标签名称和描述，保存旧名到目标名的回读结果。
4. 逐个更新开放 Issue 的标题和正文；每次更新后读取标题、正文、标签和状态。
5. 只为需要协作的关闭 Issue 增加中文摘要，不覆盖英文历史正文。
6. 更新仓库简介和 Topics 后回读公开元数据。
...
The migration is staged: review repository files first, rename labels one by one, update open Issues while preserving English text, add summaries rather than overwrite closed Issue history, update repository metadata, and read back every remote change. This does not claim complete bilingual support for the app or its companion bundles.
