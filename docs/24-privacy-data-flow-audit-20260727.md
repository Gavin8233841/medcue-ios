...
| 药品与风险列表搜索词 | 用户在药品页或风险复核页主动输入；不请求额外权限 | 查询词只保存在当前 SwiftUI 视图状态，并在内存中与已有药品、风险和来源字段做确定性匹配；不写入 SwiftData、UserDefaults 或日志 | 不发送到 Watch、Widget、Spotlight 或其他设备 | 不发送到网络、Token Broker 或本地/在线 AI | Issue #62 的搜索投影和合成测试固定本地内存边界；不形成可跨启动恢复的查询历史 |
