...
- 药品与记录合并：药品页包含清晰的服药记录入口，可进入周历、月历和每日详情；用药概览四个小卡片分别进入药品总览、今日待处理、药盒管理和服用趋势；药品分组用三张状态卡展示正在服用、服用中断和归档药物。
...
- 药品页：已改成健康式概览，集中展示药品数、待处理任务、药盒低量和今日完成率；概览四个小卡片分别跳转到对应详情，不再整体跳到记录页。药品卡片显示药盒低量提示、下次任务、记录数和剩余量。详情页支持用户选择药品照片、真机拍照上传入口、疗程起止、多提醒时间、剂量生效日期、剂量变化记录、来源说明和药盒库存编辑；药盒管理页按真实记录展示日均消耗和预计可用天数，估算结果只提示核对实物。
- 服用趋势：至少 7 个有提醒日期后，按近 7 天完成率、前一周期变化、线性斜率、忽略/稍后比例和稳定度生成客观趋势；剂量变化只作为解释上下文，提示用户结合调整前后分别观察，不生成诊断或剂量建议。
- 记录页：折叠时展示本周日历，展开后显示月历；每个日期都可点开查看当天服药详情和当天生效的剂量变化，并进入记录修正。
...
- `swift-core` Swift Testing：60 个测试通过。
