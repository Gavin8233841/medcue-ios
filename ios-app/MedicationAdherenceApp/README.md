...
- 适老模式：应用设置和完整模式今日页的醒目卡片均可切换到无标签栏的单页入口；页面复用今日页已去重、按时间排序的开放任务，只显示第一项当前任务和其余待处理数量。动作仅包含“已服用/已使用”“30 分钟后提醒”“需要帮助”；延后到期仍显示未确认。帮助号码安全保存在本机钥匙串，电话动作只请求系统打开确认界面，不写用药状态或日志；缺少号码时回到同机设置。完整模式与适老模式始终共用同一批任务和记录。
